...
#define YXSB_WATERMARK_CODE                        0x1B1  /* ??????????????????????????????????????????user data */
...
typedef struct
{
    ...
    UINT32  FirstMbInSliceFlag; //??????????skip ????????????
    UINT32  slice_vertical_position;    //slice start code ??????8bit, 01~AF
    UINT32  slice_vertical_position_extension; //????
    UINT32  previous_macroblock_address;   //??????????????????????,????slice????????????????????????????slice????????????????????????
    UINT32  intra_slice;
...
typedef struct
{
    ...
    MP2VOBUFINF   VOBufInf[MAX_QUEUE_IMAGE_NUM];       //VO????????????
