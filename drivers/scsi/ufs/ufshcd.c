...
int ufshcd_copy_query_response(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	if (hba->dev_cmd.query.descriptor &&
	    (lrbp->ucd_rsp_ptr->qr.opcode == UPIU_QUERY_OPCODE_READ_DESC || lrbp->ucd_rsp_ptr->qr.opcode == UPIU_QUERY_OPCODE_READ_HI1861_FSR)) {
...
ufshcd_transfer_rsp_status(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	if (host_byte(result) != DID_OK)
		ufshcd_print_trs(hba, 1U << (unsigned int)(lrbp->task_tag), true);
...
static int ufshcd_abort(struct scsi_cmnd *cmd)
{
	...
	if (!(reg & (1U << tag))) {
		...
		__func__, tag);
		ufshcd_print_host_regs(hba);
		ufshcd_print_trs(hba, 1U << tag, false);
	...
	if (!hba->req_abort_count) {
		...
		ufshcd_print_host_state(hba);
		ufshcd_print_pwr_info(hba);
		ufshcd_print_trs(hba, 1U << tag, true);
	} else {
		ufshcd_print_trs(hba, 1U << tag, false);
	}
	hba->req_abort_count++;
