...
static struct dwc3_otg *dwc_otg_handler;
...
int dwc3_otg_work(int evt)
{
	...
	mutex_lock(&dwc_otg_handler->lock);

	switch (evt) {
	case DWC3_OTG_EVT_ID_SET:
		ret = dwc3_set_mode(dwc_otg_handler->dwc, USB_DR_MODE_UNKNOWN);
		break;
	case DWC3_OTG_EVT_ID_CLEAR:
		ret = dwc3_set_mode(dwc_otg_handler->dwc, USB_DR_MODE_HOST);
		break;
	case DWC3_OTG_EVT_VBUS_SET:
		ret = dwc3_set_mode(dwc_otg_handler->dwc, USB_DR_MODE_PERIPHERAL);
		break;
	...
	mutex_unlock(&dwc_otg_handler->lock);
	DBG("-\n");
