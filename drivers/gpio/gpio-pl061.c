...
unsigned char pl061_readb(struct pl061_gpio *chip, unsigned offset)
{
	...
	if (adev->secure_mode) {
		arm_smccc_1_1_smc(HISI_SECURE_GPIO_REG_READ, offset, &res);
		v = (u8)res.a1;
	} else {
...
void pl061_writeb(struct pl061_gpio *chip, unsigned char v, unsigned offset)
{
	...
	if (adev->secure_mode) {
		arm_smccc_1_1_smc(HISI_SECURE_GPIO_REG_WRITE, v, offset, &res);
	} else {
		...
		int value)
{
	...
	pl061_writeb(chip, !!value << offset, (BIT(offset + 2)));
	gpiodir = pl061_readb(chip, GPIODIR);
	gpiodir |= BIT(offset);
	pl061_writeb(chip, gpiodir, GPIODIR);
